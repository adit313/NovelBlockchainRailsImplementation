class Block < ApplicationRecord
  ...
  def self.validate_newly_received_block(json_input)
    ...
    parse_input["confirmed_transactions"].each { |txn|
      txn_in_memory_to_delete = UnconfirmedTransaction.find_by(transaction_hash: txn["transaction_hash"])
      txn_in_memory_to_delete.destroy if txn_in_memory_to_delete
    }
  end
  ...
  def self.block_unappended_transaction_test(json_input)
    ...
    block_transactions.each { |txn|
      ...
      if txn["sender"] == "0000000000000000000000000000000000000000000="
        ConfirmedTransaction.coinbase_transaction_check(txn)
        coinbase_transaction_count += 1
        coinbase_amount_to_verify += txn["amount"]
      else
        txn_test = ConfirmedTransaction.generic_transaction_check(txn)
        if txn_test
          return txn_test
        end
